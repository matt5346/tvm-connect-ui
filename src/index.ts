...
export class TvmConnectUI {
    ...
    protected itemTemplate = (provider: TvmWalletProviderConfig) => {
        ...
        return html`
            ${!hasProvider && isInitializing ? html`
                <button
                    class=${classMap({
                ...
                >
                    ${provider.info.icon ? html`
                        <img class="tvm-connect-ui-item-icon" src=${provider.info.icon} alt="" />
                    ` : null}
                    <div class="tvm-connect-ui-item-info">
                        <div class="tvm-connect-ui-item-name">
                            ${provider.info.name}
                        </div>
                        <div class="tvm-connect-ui-item-desc">
                            Initializing...
                        </div>
                    </div>
                </button>
            `
            : this.isMobile && !hasProvider && universalLink ? html `
                <a
                    target="_blank"
                    rel="noopener noreferrer"
                    class="tvm-connect-ui-item"
                    href=${universalLink}
                >
                    ${provider.info.icon ? html`
                        <img class="tvm-connect-ui-item-icon" src=${provider.info.icon} alt="" />
                    ...
                    <div class="tvm-connect-ui-item-info">
                        ...
                        ${provider.info.description ? html`
                            <div class="tvm-connect-ui-item-desc">
            ...
            : !hasProvider ? html`
                <a
                    target="_blank"
                    rel="noopener noreferrer"
                    class=${classMap({'tvm-connect-ui-item-disabled': !uri && !homepage, 'tvm-connect-ui-item': true})}
            ...
            : html`
                <button
                    class="tvm-connect-ui-item"
                    @click=${this.selectProvider.bind(this, provider.id)}
                >
                    ${provider.info.icon ? html`
                        <img class="tvm-connect-ui-item-icon" src=${provider.info.icon} alt="" />
                    ` : null}
                    ...
                    ${this.providerId === provider.id ? html`
                        <div class="tvm-connect-ui-item-label">
                            Connected
                        </div>
                    ` : meta && meta.providerId === provider.id ? html`
                        <div class="tvm-connect-ui-item-label">
                            Recent
                        </div>
                    ` : null}
            ...
            <div class="tvm-connect-ui-provider">
                ...
                ${this.providerId === this.selectedProvider.id ? html`
                    <div class="tvm-connect-ui-provider-status">
                        ...
                        <button class="tvm-connect-ui-provider-btn" @click=${disconnect}>
                            Disconnect
                        </button>
                    </div>
                ` : this.selectedProviderError ? html`
                    <div class="tvm-connect-ui-provider-status">
                        <div class="tvm-connect-ui-provider-error">
                            Connection failed
                        </div>
                        <div class="tvm-connect-ui-provider-text">
                            The request was rejected, please try again
                        </div>
                        <button class="tvm-connect-ui-provider-btn" @click=${this.selectProvider.bind(this, this.selectedProvider.id)}>
                            Try again
                        </button>
                    </div>
                ` : html`
        ...
        return html`
            ...
            <div class="tvm-connect-ui-popup">
                <button class="tvm-connect-ui-close" tabIndex="0" @click=${this.hide}>
                    <svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 -960 960 960" width="24px" fill="currentColor"><path d="M480-437.85 277.08-234.92q-8.31 8.3-20.89 8.5-12.57.19-21.27-8.5-8.69-8.7-8.69-21.08 0-12.38 8.69-21.08L437.85-480 234.92-682.92q-8.3-8.31-8.5-20.89-.19-12.57 8.5-21.27 8.7-8.69 21.08-8.69 12.38 0 21.08 8.69L480-522.15l202.92-202.93q8.31-8.3 20.89-8.5 12.57-.19 21.27 8.5 8.69 8.7 8.69 21.08 0 12.38-8.69 21.08L522.15-480l202.93 202.92q8.3 8.31 8.5 20.89.19 12.57-8.5 21.27-8.7 8.69-21.08 8.69-12.38 0-21.08-8.69L480-437.85Z"/></svg>
                </button>
    ...
    protected get isInitializing(): { [providerId: string]: boolean | undefined } {
        return Object.fromEntries(this.tvmWallet.providers.map(item => (
            [item.id, item.connector.isInitializing === undefined || item.connector.isInitializing]
        )))
    }
    ...
    protected get state(): TvmConnectState {
        return {
            ...
            isNotInstalled: Object.values(this.isInitializing).every(item => item === false)
